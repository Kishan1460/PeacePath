...
export default function Login() {
  ...
  const { login } = useContext(WellnessContext);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Authentication error failed.');
    }
  };
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-8 rounded-2xl border border-gray-100 shadow-xs mt-12 animate-fade-in">
      ...
      <p className="text-center text-sm text-gray-500 mt-4">
        New here? <Link to="/register" className="text-emerald-600 font-medium">Create a safe profile account</Link>
      </p>
    </div>
  );
}